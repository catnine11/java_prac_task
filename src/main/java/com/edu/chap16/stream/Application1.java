...
public class Application1 {
    public static void main(String[] args) {
        ...
        List<Employee> employees    // Employee 객체를 담는 리스트 생성
                = Arrays.asList(   // 요소들을 리스트로 변환
        ...
        List<String> highSalaryIT = employees.stream()
                .filter(e -> e.getDepartment().endsWith("IT") && e.getSalary() >= 5000)
                .sorted(Comparator.comparing(Employee::getSalary).reversed())
                .map(Employee::getName)
                .collect(Collectors.toList());

        System.out.println("highSalaryIT = " + highSalaryIT);
